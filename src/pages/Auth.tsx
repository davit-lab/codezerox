...
const Auth = () => {
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    ...
    if (error) {
      toast.error(error.message || `${provider === 'google' ? 'Google' : 'GitHub'}-ით შესვლა ვერ მოხერხდა`);
    }
    setLoading(false);
  };
              ...
              {(mode === "login" || mode === "register") && (
                <div className="space-y-3 mb-6">
                  <button
                    ...
                    disabled={loading}
                    className="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm font-semibold transition-all hover:bg-white/10 hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
                  >
                    ...
                    {mode === "register" ? "Google-ით რეგისტრაცია" : "Google-ით შესვლა"}
                  </button>
                  ...
                  >
                    ...
                    {mode === "register" ? "GitHub-ით რეგისტრაცია" : "GitHub-ით შესვლა"}
                  </button>

                  <div className="flex items-center gap-3 my-4">
                    <div className="flex-1 h-px bg-white/10" />
                    <span className="text-xs text-stone-500 font-medium">ან</span>
                    <div className="flex-1 h-px bg-white/10" />
                  </div>
                </div>
              )}
